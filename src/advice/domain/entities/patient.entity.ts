...
export class Patient {
  ...
  setInitialDiagnosis(diagnosis: Diagnosis) {
    if (this.diagnosis)
      throw new Error('El paciente ya tiene un diagnóstico inicial.');
    this.diagnosis = diagnosis;
  }

  getDiagnosis(): Diagnosis | undefined {
  ...
  addEvaluation(evaluation: Evaluation) {
    ...
    const exists = this.evaluations.some(
      (e) => e.getDate().toDateString() === evaluation.getDate().toDateString(),
    );
    if (exists)
      throw new Error(
        `Ya existe una evaluación para la fecha ${evaluation.getDate().toDateString()}`,
      );
  ...
  getInitialDiagnosis(): Diagnosis | undefined {
    return this.diagnosis;
  }
}
